...
cast code "$APPCHAIN_PORT" --rpc-url "$PYLON_RPC_URL" | head -c 20
...
## 3) Deterministically redeploy the appchain forwarder + MigratableHumanNFT (post-reset)
...
- `AppchainReadForwarder` (appchain)
- `MigratableHumanNFT` (appchain)
...
export SEED_JSON="$(cat "$SEED_INPUT")"
